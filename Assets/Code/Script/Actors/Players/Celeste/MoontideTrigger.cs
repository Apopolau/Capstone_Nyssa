...
public class MoontideTrigger : MonoBehaviour
{
    [SerializeField] private BoxCollider moonTideCollider;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == "Enemy")
        {
            ...
            if (!enemyBeingHit.GetIsBeingHit())
            {
                ...
                //PowerStats moonTideStats = celestialPlayer.GetComponent<PowerBehaviour>().MoonTideAttackStats;
                //bool enemyIsDead;
                enemyBeingHit.SetBeingHit(true);
                //int HitPoints = Random.Range(moonTideStats.minDamage, moonTideStats.maxDamage);
                //if (currTarget)
                //{ enemyIsDead = enemyBeingHit.TakeHit(HitPoints); }
                //enemyIsDead = enemyBeingHit.TakeHit(HitPoints);
                celestialPlayer.MoonTideAttack(enemyBeingHit);
        ...
        {
            other.GetComponent<ClearDebrisTrigger>().InitiateClear();
        }
    }
